...
test.describe("Promotion ", () => {
  ...
  test("MKT/PROMO/5.2.2.1/2 - 推广规则：相等于数量3就可以全单，以折实价享有20%的折扣", async () => {
    ...
    await onlineStore.page.waitForTimeout(2000);
    await onlineStore.searchInput.fill("篮球鞋");
    ...
    await onlineStore.page.waitForTimeout(2000);
    await shoppingCart.checkPromotionText("满800减80", "HK$ 1840");
    await shoppingCart.checkPromotionText("购买3件商品, 全单20%折扣", "HK$ 1840");
    await shoppingCart.quanitySniper.fill("6");
    await onlineStore.page.waitForTimeout(2000);
    await shoppingCart.checkPromotionText("满800减80", "HK$ 3760");
    await shoppingCart.checkPromotionText("购买3件商品, 全单20%折扣", "HK$ 3760");
    await shoppingCart.goCheckOut.click();
    await onlineStore.page.waitForTimeout(2000);
    await onlineStore.verifyGrandTotal("3760");
