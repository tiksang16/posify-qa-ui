...
test.describe("Promotion ", () => {
  ...
  test("MKT/PROMO/5.2.2.1/6 - 买相等于数量2就可以获得15购物金，实际数量买4", async () => {
    await marketingPage.activatePromotion("买相等于数量2送获得15购物金");
    ...
    // expect(onlineStore.page.getByText("獲得/扣除: 15").first()).toBeVisible();

    await marketingPage.deactivatePromotion("买相等于数量2送获得15购物金");
  });
});
