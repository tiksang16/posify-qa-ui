...
test.describe("Promotion ", () => {
  ...
  test("MKT/PROMO/5.2.2.1/4 - 买相等于数量2就可以获得2倍积分，实际数量买4", async () => {
    await marketingPage.activatePromotion("买相等于数量2就可以获得2倍积分");
    ...
    await onlineStore.verifyGrandTotal("4000");
    ...
    // expect(onlineStore.page.getByText("獲得/扣除: 56").first()).toBeVisible();

    await marketingPage.deactivatePromotion("买相等于数量2就可以获得2倍积分");
  });
});
