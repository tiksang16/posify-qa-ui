...
test.describe("Promotion ", () => {
  ...
  test("MKT/PROMO/5.2.2.1/8 - 推广规则：买满不少于数量3就可以享受100的固定折扣，去没启用推广优惠下的店铺使用", async () => {
  
    await marketingPage.activatePromotion("买满不少于数量3减100，去没启用推广优惠下的店铺使用");
    ...
    await marketingPage.deactivatePromotion("买满不少于数量3减100，去没启用推广优惠下的店铺使用");
  });
